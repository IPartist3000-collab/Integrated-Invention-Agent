    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/${validModelName}:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ 
          ...
          // 일부 환경에서 tools 에러가 날 수 있어 최소 사양으로 전송합니다.
        })
      });
      ...
      <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-slate-200">
        <div className="flex gap-4 mb-6 sticky top-0 z-10">
          <button onClick={() => setMode('IDF')} className={`flex-1 p-4 rounded-xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'IDF' ? 'border-emerald-500 bg-white text-emerald-700 shadow-md' : 'bg-slate-50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-50 uppercase tracking-widest">Step 1. Identify</span>
          ...
          <button onClick={() => setMode('SPEC')} className={`flex-1 p-4 rounded-xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'SPEC' ? 'border-blue-500 bg-white text-blue-700 shadow-md' : 'bg-slate-50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-50 uppercase tracking-widest">Step 2. Legalize</span>
        ...
        <div className="bg-white shadow-2xl p-12 lg:p-20 rounded-xl min-h-screen border border-slate-300">
          {mode === 'IDF' ? (
            <div className="space-y-8">
              <h2 className="text-2xl font-black text-emerald-900 border-b-4 border-emerald-900 pb-2 uppercase tracking-tighter italic">Invention Disclosure Form</h2>
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2 p-6 bg-slate-50 rounded-lg border-l-8 border-emerald-500"><h3 className="text-xs font-black text-emerald-700 mb-2 uppercase tracking-widest">Title of Invention</h3><p className="text-xl font-bold">{idf.title || "발명의 명칭"}</p></div>
                <div className="p-6 bg-emerald-50 rounded-lg border border-emerald-100"><h3 className="text-xs font-black text-emerald-700 mb-2 uppercase tracking-widest">Core Novelty</h3><p className="text-sm leading-relaxed">{idf.novelty}</p></div>
                <div className="p-6 bg-blue-50 rounded-lg border border-blue-100"><h3 className="text-xs font-black text-blue-700 mb-2 uppercase tracking-widest">Commercial Value</h3><p className="text-sm leading-relaxed">{idf.commercial_value}</p></div>
                <div className="col-span-2 p-6 border rounded-lg bg-white"><h3 className="text-xs font-black text-slate-500 mb-2 uppercase tracking-widest">Technical Description</h3><p className="text-sm whitespace-pre-wrap leading-relaxed">{idf.technical_key}</p></div>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              <h1 className="text-xl font-bold mb-8 border-b-2 border-black pb-1 text-center uppercase tracking-tighter">【명세서】</h1>
              <div className="mb-4"><h3 className="font-bold text-sm">【발명(고안)의 명칭】</h3><p className="pl-4 text-indigo-800 font-bold">{spec.title}</p></div>
              <div className="mb-4"><h3 className="font-bold text-sm">【해결하려는 과제】</h3><p className="pl-4 text-sm">{spec.problem}</p></div>
              <div className="mt-10 p-6 bg-red-50/30 border border-red-100 rounded-xl">
                <h3 className="font-bold text-red-800 mb-4 flex items-center gap-2">【특허청구범위】</h3>
                {spec.claims.map((c, i) => ( <div key={i} className="mb-4 last:mb-0"><h4 className="font-bold text-xs text-red-600 mb-1">【청구항 {i+1}】</h4><p className="text-sm pl-2 leading-relaxed text-slate-800 border-l-2 border-red-100">{c}</p></div> ))}
              </div>
            </div>
          )}
        </div>
