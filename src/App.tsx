...
export default function App() {
  ...
  const [messages, setMessages] = useState([{ role: 'bot', content: '✅ 시스템이 준비되었습니다. 아이디어를 입력하면 IDF부터 명세서까지 자동 설계합니다.' }]);
  const [input, setInput] = useState('');
  const [validModelName, setValidModelName] = useState(null);

  // === 2. 엔진 초기화: 모델 자동 감지 (오류 방지 핵심) ===
  useEffect(() => {
  ...
  const handleDownload = () => {
    ...
    const htmlContent = `<html><head><meta charset='utf-8'></head><body><h1>【명세서】</h1><h2>【발명의 명칭】</h2><p>${spec.title}</p><h2>【청구범위】</h2>${spec.claims.map((c, i) => `<p>【청구항 ${i+1}】<br/>${c}</p>`).join('')}</body></html>`;
  ...
  const handleSend = async () => {
    ...
    const specInstructions = `당신은 20년 경력의 대한민국 특허청 심사관입니다. [청구항 제1항은 필수 구성요소를 모두 포함한 하나의 문장으로 작성할 것] JSON 응답: { "title": "명칭", "technical_field": "기술분야", "background": "배경기술", "problem": "과제", "solution": "수단", "effects": "효과", "claims": ["항1", "항2"], "abstract": "요약" }`;
    ...
    try {
      ...
      if (specJson) {
        ...
        setMode('SPEC');
        setMessages(prev => [...prev, { role: 'bot', content: `✅ 전주기 설계가 완료되었습니다.` }]);
      }
    } catch (error) {
        ...
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder="Password" 
              className={`w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 ${loginError ? 'border-red-500' : 'border-slate-300'}`} autoFocus />
            <button type="submit" className="w-full bg-indigo-700 text-white font-bold p-3 rounded-xl hover:bg-indigo-800 transition-all shadow-lg">접속하기</button>
          </form>
        </div>
      </div>
    );
      ...
      <div className="w-[35%] flex flex-col bg-white border-r shadow-2xl z-20">
        <div className="p-5 bg-indigo-900 text-white flex justify-between items-center shadow-lg">
          ...
          <div className="flex bg-indigo-800 rounded-md p-1 font-bold text-[10px]">
            <button onClick={() => setMode('IDF')} className={`px-4 py-1 rounded ${mode === 'IDF' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400'}`}>IDF</button>
            <button onClick={() => setMode('SPEC')} className={`px-4 py-1 rounded ${mode === 'SPEC' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-400'}`}>SPEC</button>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-slate-200">
        <div className="flex gap-4 mb-8 sticky top-0 z-10 bg-slate-200 py-2 justify-between items-center">
          <div className="flex gap-4 flex-1">
            <button onClick={() => setMode('IDF')} className={`flex-1 p-4 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'IDF' ? 'border-emerald-500 bg-white text-emerald-700 shadow-xl' : 'bg-slate-50/80 text-slate-400 border-transparent'}`}>
              <span className="text-[10px] font-black opacity-40 uppercase italic">Step 1</span>
              <div className="flex items-center gap-2 font-black text-sm"><Lightbulb size={18}/> 발명 특정 (IDF)</div>
            </button>
            <button onClick={() => setMode('SPEC')} className={`flex-1 p-4 rounded-xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'SPEC' ? 'border-blue-500 bg-white text-blue-700 shadow-xl' : 'bg-slate-50/80 text-slate-400 border-transparent'}`}>
              <span className="text-[10px] font-black opacity-40 uppercase italic">Step 2</span>
          ...
          {mode === 'IDF' ? (
            ...
            <div className="space-y-8 animate-in fade-in duration-700">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-2"><span className="font-bold text-slate-900">【해결하려는 과제】</span><p className="text-sm text-slate-700 leading-relaxed pl-4 border-l-2 border-slate-100 whitespace-pre-wrap">{spec.problem}</p></div>
                <div className="flex flex-col gap-2"><span className="font-bold text-slate-900">【과제의 해결수단】</span><p className="text-sm text-slate-700 leading-relaxed pl-4 border-l-2 border-slate-100 whitespace-pre-wrap">{spec.solution}</p></div>
                <div className="mt-12 p-8 bg-red-50/20 border-2 border-red-100 rounded-2xl shadow-inner text-justify">
                  ...
                  {spec.claims.map((c, i) => ( 
                    <div key={i} className="mb-6 last:mb-0">
                      ...
                      <p className="text-sm pl-4 leading-8 text-slate-800 border-l-2 border-red-100 font-medium whitespace-pre-wrap">{c}</p>
                    </div> 
                  ))}
                </div>
                <div className="flex flex-col gap-2 pt-6 border-t border-slate-100"><span className="font-bold text-slate-900 font-bold">【요약】</span><p className="text-sm text-slate-600 leading-relaxed pl-4 whitespace-pre-wrap italic">{spec.abstract}</p></div>
              </div>
            </div>
          )}
        </div>
