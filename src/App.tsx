...
export default function App() {
      ...
      try {
        ...
        if (data.models) {
          ...
          if (targetModel) {
            ...
            setMessages([{ role: 'bot', content: '✅ 엔진 연결 성공! 222nm 살균 장치와 같은 아이디어를 입력하세요.' }]);
          }
        }
      } catch (error) {
  ...
  const handleSend = async () => {
    ...
    // [프롬프트 설계] IDF와 SPEC 각각의 전문 페르소나 설정
    ...
    const specInstructions = `당신은 20년 경력의 대한민국 특허청 심사관입니다. 제공된 IDF 데이터를 바탕으로 법률적으로 견고한 명세서를 작성하세요. 
    1. 독립항은 필수 구성요소를 모두 포함한 하나의 문장으로 구성할 것. 
    2. 종속항은 독립항을 구체화할 것. 
    다음 JSON 형식으로 응답하세요: { "title": "명칭", "technical_field": "분야", "background": "배경", "problem": "과제", "solution": "해결수단", "effects": "효과", "claims": ["청구항1", "청구항2"], "abstract": "요약" }`;
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', content: `❌ 오류 발생: ${error.message}` }]);
    } finally {
    ...
    <div className="flex h-screen w-full bg-slate-100 font-sans overflow-hidden">
      ...
      <div className="w-[35%] flex flex-col bg-white border-r shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
          ...
          {isGenerating && <div className="flex gap-2 text-slate-400 text-xs items-center pl-2"><Loader2 size={14} className="animate-spin"/> AI가 문서 설계 중...</div>}
        </div>
        <div className="p-4 border-t bg-white flex gap-2">
          <input className="flex-1 p-2 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="메시지를 입력하세요..." disabled={!validModelName} />
          <button onClick={handleSend} className="p-2 bg-indigo-700 text-white rounded-xl shadow-lg hover:bg-indigo-800" disabled={!validModelName}><Send size={18}/></button>
        </div>
      </div>

      {/* 오른쪽: 문서 프리뷰 (65%) */}
      <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-slate-200 font-sans">
        <div className="flex gap-4 mb-6 sticky top-0 z-10">
          <button onClick={() => setMode('IDF')} className={`flex-1 p-5 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'IDF' ? 'border-emerald-500 bg-white text-emerald-700 shadow-xl' : 'bg-slate-50/50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-40 tracking-widest uppercase italic">Step 1. Identify</span>
          ...
          <button onClick={() => setMode('SPEC')} className={`flex-1 p-5 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'SPEC' ? 'border-blue-500 bg-white text-blue-700 shadow-xl' : 'bg-slate-50/50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-40 tracking-widest uppercase italic">Step 2. Legalize</span>
            <div className="flex items-center gap-2 font-black text-base"><Zap size={20}/> 권리 보호 (명세서)</div>
          </button>
        </div>

        <div className="bg-white shadow-2xl p-12 lg:p-20 rounded-xl min-h-screen border border-slate-300 mx-auto w-full max-w-4xl leading-relaxed">
          {mode === 'IDF' ? (
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="p-8 bg-emerald-50/50 rounded-xl border border-emerald-100"><h3 className="text-xs font-black text-emerald-700 mb-3 uppercase tracking-widest">Core Novelty</h3><p className="text-sm font-medium">{idf.novelty}</p></div>
                <div className="p-8 bg-blue-50/50 rounded-xl border border-blue-100"><h3 className="text-xs font-black text-blue-700 mb-3 uppercase tracking-widest">Commercial Value</h3><p className="text-sm font-medium">{idf.commercial_value}</p></div>
                ...
                <div className="mt-12 p-8 bg-red-50/20 border-2 border-red-100 rounded-2xl shadow-inner">
                  ...
                  {spec.claims.map((c, i) => ( 
                    <div key={i} className="mb-6 last:mb-0">
                      <h4 className="font-black text-xs text-red-600 mb-2 underline decoration-red-200 underline-offset-4">【청구항 {i+1}】</h4>
                      <p className="text-sm pl-4 leading-7 text-slate-800 border-l-2 border-red-100 font-medium">{c}</p>
                    </div> 
                  ))}
                </div>
              </div>
