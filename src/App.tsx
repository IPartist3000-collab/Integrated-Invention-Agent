...
export default function App() {
  ...
  // [수정] null 대신 즉시 사용 가능한 모델 경로를 초기값으로 설정합니다.
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/${validModelName}:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ 
          ...
          // [수정] 'not supported' 오류 방지를 위해 tools 옵션을 제거하고 텍스트 생성에 집중합니다.
        })
      });
          ...
          {mode === 'IDF' ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="col-span-2 p-6 border rounded-lg bg-white shadow-inner"><h3 className="text-xs font-black text-slate-500 mb-2 uppercase">Technical Description</h3><p className="text-sm whitespace-pre-wrap">{idf.technical_key}</p></div>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="mt-10 p-6 bg-red-50/30 border border-red-100 rounded-xl">
                ...
                {spec.claims.map((c, i) => ( <div key={i} className="mb-4"><h4 className="font-bold text-xs text-red-600 mb-1">【청구항 {i+1}】</h4><p className="text-sm pl-2 leading-relaxed border-l-2 border-red-100">{c}</p></div> ))}
              </div>
            </div>
          )}
        </div>
