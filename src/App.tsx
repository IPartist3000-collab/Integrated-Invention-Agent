...
export default function App() {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', content: '지능형 발명 에이전트입니다. 222nm 단일파장 살균 기술과 같은 아이디어를 입력하여 발명을 특정해 보세요.' }
  ]);
  const [input, setInput] = useState('');

  // === 2. 핵심 로직: 에러 방지 및 모델 직접 지정 ===
  const handleSend = async () => {
    ...
    // 프롬프트 전략: 따옴표 에러 방지를 위해 백틱 사용
    const idfPrompt = `당신은 전문 IP 컨설턴트입니다. 다음 기술 아이디어를 분석하여 반드시 JSON 형식으로만 응답하세요: { "title": "명칭", "problem": "기존 한계", "novelty": "핵심 신규성", "technical_key": "구현 방법", "effects": "효과", "commercial_value": "상업성", "summary": "요약" }`;
    
    const specInstructions = `당신은 20년 경력의 대한민국 특허청 심사관입니다. 다음 IDF 데이터를 바탕으로 법률적 명세서를 설계하세요. 반드시 JSON 형식으로 응답하세요: { "title": "명칭", "technical_field": "분야", "background": "배경", "problem": "과제", "solution": "해결수단", "effects": "효과", "claims": ["청구항1", "청구항2"], "abstract": "요약" }`;
    ...
    try {
      ...
      // 모델명을 gemini-1.5-flash로 고정하여 안정성 확보
      ...
      if (!data.candidates || !data.candidates[0]) {
        throw new Error("API 응답이 없습니다. API 키를 확인하세요.");
      }
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'bot', content: `❌ 오류: 다시 시도해 주세요.` }]);
    } finally {
      setIsGenerating(false);
    }
  };

  // IDF 데이터를 명세서 작성의 기반으로 자동 주입
  const linkIdfToSpec = () => {
    setMode('SPEC');
    setInput(`${idf.title}에 대해 작성된 IDF의 신규성을 바탕으로 명세서를 설계해줘.`);
  };

  // === 3. UI 레이아웃: 이전의 넓고 전문적인 포맷 유지 ===
  return (
      ...
      <div className="w-[35%] flex flex-col bg-white border-r shadow-2xl">
        <div className="p-5 bg-indigo-900 text-white flex justify-between items-center">
          ...
          <div className="flex bg-indigo-800 rounded-md p-1">
            <button onClick={() => setMode('IDF')} className={`px-4 py-1 text-[11px] font-bold rounded ${mode === 'IDF' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400'}`}>IDF</button>
            <button onClick={() => setMode('SPEC')} className={`px-4 py-1 text-[11px] font-bold rounded ${mode === 'SPEC' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-400'}`}>SPEC</button>
          </div>
        </div>
        ...
        <div className="p-4 border-t bg-white flex gap-2">
          <input className="flex-1 p-2 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 shadow-inner" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="아이디어를 입력하세요..." />
          <button onClick={handleSend} className="p-2 bg-indigo-700 text-white rounded-xl shadow-lg hover:bg-indigo-800"><Send size={18}/></button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-slate-200">
        <div className="flex gap-4 mb-6 sticky top-0 z-10">
          <button onClick={() => setMode('IDF')} className={`flex-1 p-5 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'IDF' ? 'border-emerald-500 bg-white text-emerald-700 shadow-xl' : 'bg-slate-50/50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-40 tracking-widest">STEP 1. IDENTIFY</span>
          ...
          <button onClick={() => setMode('SPEC')} className={`flex-1 p-5 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${mode === 'SPEC' ? 'border-blue-500 bg-white text-blue-700 shadow-xl' : 'bg-slate-50/50 text-slate-400 border-transparent'}`}>
            <span className="text-[10px] font-black opacity-40 tracking-widest">STEP 2. LEGALIZE</span>
            <div className="flex items-center gap-2 font-black text-base"><Zap size={20}/> 권리 보호 (명세서)</div>
          </button>
        </div>

        <div className="bg-white shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] p-12 lg:p-20 rounded-xl min-h-screen border border-slate-300 mx-auto w-full max-w-4xl">
          {mode === 'IDF' ? (
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="col-span-2 p-8 bg-slate-50 rounded-xl border-l-[12px] border-emerald-500 shadow-sm"><h3 className="text-xs font-black text-emerald-700 mb-3 uppercase tracking-widest">Title of Invention</h3><p className="text-2xl font-bold text-slate-800">{idf.title || "아이디어를 입력해 주세요"}</p></div>
                <div className="p-8 bg-emerald-50/50 rounded-xl border border-emerald-100"><h3 className="text-xs font-black text-emerald-700 mb-3 uppercase tracking-widest">Core Novelty</h3><p className="text-sm leading-relaxed font-medium">{idf.novelty}</p></div>
                <div className="p-8 bg-blue-50/50 rounded-xl border border-blue-100"><h3 className="text-xs font-black text-blue-700 mb-3 uppercase tracking-widest">Commercial Value</h3><p className="text-sm leading-relaxed font-medium">{idf.commercial_value}</p></div>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="space-y-6">
                ...
                <div className="mt-12 p-8 bg-red-50/20 border-2 border-red-100 rounded-2xl">
                  ...
                  )) : <p className="text-slate-400 text-sm italic">청구항을 생성 중입니다...</p>}
                </div>
              </div>
            </div>
          )}
