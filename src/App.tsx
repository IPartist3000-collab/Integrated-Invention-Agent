...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'bot', content: "❌ 엔진 연결 중 오류가 발생했습니다. API 설정을 확인해 주세요." }]);
    } finally {
        ...
        <div className="bg-white shadow-2xl p-12 lg:p-20 rounded-xl min-h-screen border border-slate-300">
          {mode === 'IDF' ? (
            <div className="space-y-8">
              <h2 className="text-2xl font-black text-emerald-900 border-b-4 border-emerald-900 pb-2 uppercase italic tracking-tighter">Invention Disclosure Form</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="mt-10 p-6 bg-red-50/30 border border-red-100 rounded-xl">
                ...
                {spec.claims.map((c, i) => ( <div key={i} className="mb-4"><h4 className="font-bold text-xs text-red-600 mb-1">【청구항 {i+1}】</h4><p className="text-sm pl-2 leading-relaxed text-slate-800 border-l-2 border-red-100">{c}</p></div> ))}
              </div>
            </div>
          )}
        </div>
